...
func (i *Indexer) loadHistoricalEvents(_currentBlock uint64, wg *sync.WaitGroup) error {
	i.logger.Info("Loading historical events...")
		...
		for _, eventName := range contractConfig.Events {
			...
			go func(contractConfig config.ContractConfig, event abi.Event) {
				...
				case <-i.ctx.Done():
					return
				case <-i.stopChan:
					i.logger.Info("Stop signal received, stopping historical events loading")
					return
				case err := <-errorChan:
					i.logger.Error("Error in historical events loading: %v", err)
					i.stopChan <- StopChannel{
						blockNumber: 0,
						blockHash:   "",
						isReorg:     false,
					}
				...
				if err != nil {
					...
					errorChan <- fmt.Errorf("failed to get last indexed block: %w, event: %s, contract: %s", err, event.Name, contractConfig.Address)
					return
				...
				i.logger.Debug("Fetching historical events for %s on contract %s, from block %d to block %d", event.Name, contractConfig.Address, *lastIndexedBlock, _currentBlock)
				...
				if err != nil {
					i.logger.Error("Failed to get historical events: %v, event: %s, contract: %s", err, event.Name, contractConfig.Address)
					errorChan <- fmt.Errorf("failed to get historical events: %w, event: %s, contract: %s", err, event.Name, contractConfig.Address)
					return
				}
