		...
		for _, _eventName := range contract.Events {
			...
			go func(event abi.Event) {
				...
				for {
					...
					select {
					case <-i.ctx.Done():
						i.logger.Info("Context cancelled, stopping subscription for %s on contract %s", event.Name, contractAddress.String())
						// Don't send error on context cancellation during shutdown
						return
					case <-i.stopChan:
						i.logger.Info("Stop signal received, stopping subscription for %s on contract %s", event.Name, contractAddress.String())
						return
					case err := <-errorChan:
						i.logger.Error("Error in subscription: %v for %s on contract %s", err, event.Name, contractAddress.String())
					...
					if err != nil {
						errorChan <- fmt.Errorf("failed to subscribe to event %s for contract %s: %w", event.Name, contractAddress.String(), err)
						return
					}
...
func (i *Indexer) subscribeToEvent(contractAddress common.Address, event abi.Event) error {
	...
	for {
		...
		case <-i.ctx.Done():
			i.logger.Info("Context cancelled, stopping subscription")
			sub.Unsubscribe()
			return nil
		}
	}
